...
const BG = `
  radial-gradient(120% 60% at 100% 0%, rgba(255,220,205,0.55) 0%, transparent 55%),
  radial-gradient(80% 60% at 0% 100%, rgba(232,106,79,0.65) 0%, transparent 60%),
  linear-gradient(135deg, #F08060 0%, #E86A4F 55%, #C84A30 100%)
`;
const SHADOW = 'rgba(120, 30, 12, 0.35)';
const ACCENT = '#7A1F12';

export function LightRedbrick() {
  return <ThemedShell bg={BG} shadow={SHADOW} accent={ACCENT} label="LIGHT REDBRICK" />;
}
...
const glass = (shadow: string, strong = false): React.CSSProperties => ({
  ...
  boxShadow: `0 1px 0 rgba(255,255,255,0.35) inset, 0 -1px 0 rgba(255,255,255,0.06) inset, 0 16px 36px -14px ${shadow}, 0 4px 10px -3px ${shadow}`,
});
...
function ThemedShell({ bg, shadow, accent, label }: { bg: string; shadow: string; accent: string; label: string }) {
  ...
  const stats = [
    { label: 'Total Revenue', value: '$2.84M', delta: '+12.4%', color: '#fff' },
    { label: 'Active Trades', value: '47', delta: '+8', color: '#fff' },
    { label: 'In Transit', value: '23.5K MT', delta: '+3.2%', color: '#fff' },
    { label: 'Escrow Locked', value: '$840K', delta: '4 deals', color: '#fff' },
  ];
  return (
        ...
        <aside className="w-[232px] shrink-0 p-3 flex flex-col gap-2 h-screen sticky top-0">
          ...
          <div className="rounded-2xl p-2.5" style={glass(shadow)}>
            <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.18)' }}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: `linear-gradient(135deg, ${accent}, ${accent}aa)` }}>CP</div>
        ...
        <main className="flex-1 min-w-0 p-3 pr-4 flex flex-col gap-3">
          <header className="rounded-2xl px-5 h-14 flex items-center gap-4" style={glass(shadow)}>
            ...
            <div className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-xl" style={{ background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)' }}>
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold" style={{ background: `linear-gradient(135deg, ${accent}, ${accent}aa)` }}>C</div>
          ...
          <section className="grid grid-cols-3 gap-3">
            <div className="col-span-2 rounded-2xl p-5" style={glass(shadow)}>
              ...
              <svg viewBox="0 0 600 180" className="w-full h-44">
                ...
                <path d="M0,140 C60,110 120,130 180,90 C240,55 300,80 360,55 C420,32 480,60 540,38 L600,32 L600,180 L0,180 Z" fill="url(#lg1)" />
