...
export interface IStorage {
  ...
  // KYC Drafts
  getKycDraft(userId: string, submissionType: string): Promise<KycDraft | undefined>;
  upsertKycDraft(userId: string, submissionType: string, draftData: Record<string, any>): Promise<KycDraft>;
  deleteKycDraft(userId: string, submissionType: string): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getFinatradesPersonalKycPaginated(options: { status?: string; limit?: number; offset?: number }): Promise<{ data: Partial<FinatradesPersonalKyc>[]; total: number }> {
    ...
    const lightweightColumns = {
      ...
      dateOfBirth: finatradesPersonalKyc.dateOfBirth,
      country: finatradesPersonalKyc.country,
      city: finatradesPersonalKyc.city,
      address: finatradesPersonalKyc.address,
      postalCode: finatradesPersonalKyc.postalCode,
      nationality: finatradesPersonalKyc.nationality,
      occupation: finatradesPersonalKyc.occupation,
      sourceOfFunds: finatradesPersonalKyc.sourceOfFunds,
      idFrontUrl: finatradesPersonalKyc.idFrontUrl,
  ...
  async getKycDraft(userId: string, submissionType: string): Promise<KycDraft | undefined> {
    const [draft] = await db.select().from(kycDrafts).where(
      and(eq(kycDrafts.userId, userId), eq(kycDrafts.submissionType, submissionType))
    );
  ...
  async deleteKycDraft(userId: string, submissionType: string): Promise<void> {
    await db.delete(kycDrafts).where(
      and(eq(kycDrafts.userId, userId), eq(kycDrafts.submissionType, submissionType))
    );
  }
