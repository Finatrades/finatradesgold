...
function KnowledgeBaseManagement() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'archived': return 'bg-muted text-foreground/85';
      default: return 'bg-muted text-foreground/85';
    }
  };
...
function ConversationsPanel() {
  ...
  return (
    <>
      ...
      <div className="h-[calc(100vh-8rem)] flex bg-card rounded-xl border border-border overflow-hidden shadow-sm">
        
        {/* Sidebar - Chat List */}
        <div className="w-80 border-r border-border flex flex-col bg-muted/40/50">
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredSessions.map(session => (
              <div 
                ...
                className={`p-4 border-b border-border/60 cursor-pointer hover:bg-muted transition-colors ${
              ...
              >
                <div className="flex justify-between items-start mb-1">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className={`font-semibold text-sm ${currentSession?.id === session.id ? 'text-purple-900' : 'text-foreground'}`}>
                        {session.userName}
                      </h3>
            ...
            <>
              ...
              <div className="h-16 border-b border-border flex items-center justify-between px-6 bg-card">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-foreground">{currentSession.userName}</h3>
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${currentSession.isOnline ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`} />
                      <span className="text-xs text-muted-foreground">{currentSession.isOnline ? 'Online' : 'Offline'}</span>
                ...
                <div className="flex items-center gap-2">
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className={`hover:text-muted-foreground ${activeCall ? 'text-red-500' : 'text-muted-foreground/70'}`}
                  ...
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className={`hover:text-muted-foreground ${activeCall?.callType === 'video' ? 'text-red-500' : 'text-muted-foreground/70'}`}
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-muted/40/30">
              ...
              <div className="p-4 border-t border-border bg-card">
                <div className="flex items-center gap-2">
                  <Input 
                    ...
                    className="flex-1 bg-muted/40 border-border focus:border-purple-500 focus:ring-purple-500/20"
                    value={messageInput}
