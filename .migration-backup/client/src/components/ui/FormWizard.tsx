...
export function FormWizard({
  ...
  const progressPct = steps.length > 1 ? Math.round((currentIndex / (steps.length - 1)) * 100) : 0;

  const hasNav = !!(onBack || onNext);

  if (orientation === 'horizontal') {
    return (
      <div className={`flex flex-col gap-3 ${className}`} data-testid={dataTestId ?? "form-wizard-horizontal"}>
        {showProgress && (
        ...
        {hasNav && (
          <div className="flex justify-between mt-1">
            <Button
              ...
              size="sm"
              onClick={onBack}
              disabled={isBackDisabled || !onBack}
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      <nav className={`flex flex-col gap-1`} data-testid={dataTestId ?? "form-wizard-vertical"}>
        {steps.map((step, idx) => {
          const isActive = step.id === currentStep;
          const isCompleted = step.isComplete || idx < currentIndex;
          const isClickable = !step.isLocked && (isCompleted || isActive) && !!onStepChange;
