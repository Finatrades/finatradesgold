...
export default function FinaBridgeManagement() {
  ...
  const getFraudScoreBadgeColor = (score: number | null) => {
    if (score === null) return 'bg-muted text-foreground/85';
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          ...
          <Card className={stats.tierPending > 0 ? 'border-amber-300 bg-amber-50' : ''}>
            <CardContent className="p-4 text-center">
              <p className={`text-2xl font-bold ${stats.tierPending > 0 ? 'text-amber-600' : 'text-muted-foreground/70'}`}>{stats.tierPending}</p>
                          ...
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <Badge className={room.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-muted text-foreground/85'}>
                                  {room.status}
                                </Badge>
                                <Button size="sm" variant="ghost">
                                  <Eye className="w-4 h-4" />
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            {request.aiFraudScore && (
                              <div className="p-3 bg-muted/40 rounded-lg">
                                <p className="text-xs font-semibold text-muted-foreground mb-1">Fraud Risk Score</p>
                                <p className={`text-2xl font-bold ${getFraudScoreTextColor(request.aiFraudScore)}`}>
                                  {parseFloat(request.aiFraudScore).toFixed(1)} / 100
                                </p>
                                <p className="text-xs text-muted-foreground">{parseFloat(request.aiFraudScore) <= 20 ? 'Low risk' : parseFloat(request.aiFraudScore) <= 50 ? 'Medium risk' : 'High risk'}</p>
                              </div>
                            )}
                          </div>
                          {aiData && (
                            <div className="p-3 bg-muted/40 rounded-lg">
                              <p className="text-xs font-semibold text-muted-foreground mb-2">AI Extracted Document Data</p>
                              <pre className="text-xs text-foreground/85 whitespace-pre-wrap overflow-auto max-h-40">{JSON.stringify(aiData, null, 2)}</pre>
                            </div>
                          )}
                        </div>
                      )}
                        ...
                        <div className="mt-4 pt-4 border-t space-y-3">
                          <div className="grid grid-cols-3 gap-3">
                            ...
                            <div className="p-3 bg-muted/40 rounded-lg text-center">
                              <p className="text-xs text-muted-foreground font-semibold">Fraud Score</p>
                              <p className={`text-xl font-bold ${getFraudScoreTextColor(request.aiFraudScore)}`}>
                          ...
                          {aiData && (
                            <div className="p-3 bg-muted/40 rounded-lg">
                              <p className="text-xs font-semibold text-muted-foreground mb-2">AI Extracted Data</p>
                              <pre className="text-xs text-foreground/85 whitespace-pre-wrap overflow-auto max-h-40">{JSON.stringify(aiData, null, 2)}</pre>
                            </div>
                          )}
                        </div>
              ...
              <div className="flex h-[75vh] overflow-hidden">
                ...
                <div className="w-1/2 border-r flex flex-col">
                  <div className="px-4 py-2 bg-muted/40 border-b">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Supporting Document</p>
                  </div>
                ...
                <div className="w-1/2 flex flex-col overflow-hidden">
                  <div className="px-4 py-2 bg-muted/40 border-b">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">AI Report & Review History</p>
                  </div>
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    {docViewRequest.aiExtractedData && (() => {
                      ...
                      return (
                        <div className="space-y-2">
                          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">AI Extracted Fields</p>
                          {fields ? (
                            <div className="space-y-1">
                              {Object.entries(fields).map(([key, val]) => (
                                <div key={key} className="flex justify-between text-xs p-2 bg-muted/40 rounded border">
                            ...
                            <pre className="text-xs text-foreground/85 whitespace-pre-wrap overflow-auto max-h-32 p-2 bg-muted/40 rounded">{JSON.stringify(aiData, null, 2)}</pre>
                          )}
                        </div>
                      );
                    })()}
