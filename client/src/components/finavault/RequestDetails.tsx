...
export default function RequestDetails({ request, onClose, onCancel }: RequestDetailsProps) {
  ...
  const handleDownloadDocument = (doc: { id: string; type: string; name: string }) => {
    const pdf = new jsPDF();
    ...
    pdf.setFontSize(18);
    pdf.text('Finatrades FinaVault', pageWidth / 2, 18, { align: 'center' });
    
    pdf.setTextColor(0, 0, 0);
    pdf.setFontSize(14);
    pdf.text(doc.type.toUpperCase(), pageWidth / 2, 50, { align: 'center' });
    
    pdf.setFontSize(10);
    pdf.setTextColor(100, 100, 100);
    pdf.text(`Document: ${doc.name}`, 20, 70);
    pdf.text(`Document ID: ${doc.id}`, 20, 78);
    pdf.text(`Request ID: ${request.id}`, 20, 86);
    ...
    pdf.setTextColor(0, 0, 0);
    pdf.setFontSize(12);
    pdf.text('Deposit Details', 20, 110);
    pdf.setFontSize(10);
    pdf.text(`Vault Location: ${request.vaultLocation}`, 20, 120);
    pdf.text(`Deposit Type: ${request.depositType}`, 20, 128);
    pdf.text(`Total Weight: ${request.totalDeclaredWeightGrams}g`, 20, 136);
    pdf.text(`Submitted: ${new Date(request.submittedAt).toLocaleDateString()}`, 20, 144);
    
    pdf.setFontSize(12);
    pdf.text('Items Deposited', 20, 160);
    pdf.setFontSize(10);
    let yPos = 170;
    request.items.forEach((item) => {
      pdf.text(`• ${item.quantity}x ${item.itemType} (${item.totalWeightGrams}g)`, 25, yPos);
      yPos += 8;
    });
    
    pdf.setFontSize(8);
    pdf.setTextColor(150, 150, 150);
    pdf.text('Generated by Finatrades FinaVault', pageWidth / 2, 280, { align: 'center' });
    pdf.text(new Date().toLocaleDateString(), pageWidth / 2, 287, { align: 'center' });
    
    pdf.save(`${doc.type.replace(/\s+/g, '_')}_${doc.id}.pdf`);
    
    toast({
      title: "Document Downloaded",
      description: `${doc.type} has been saved as PDF.`
    });
  };
