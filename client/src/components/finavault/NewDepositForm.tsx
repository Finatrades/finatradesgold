        ...
        <Card className="bg-white/5 border-white/10">
          ...
          <CardContent className="space-y-6">
            {items.map((item, index) => (
              <div key={item.id} className="p-4 rounded-lg bg-white/5 border border-white/5 relative">
                <div className="absolute top-2 right-2 text-xs text-white/20 font-mono">#{index + 1}</div>
                  ...
                  <div className="space-y-2">
                   ...
                   <div className="flex items-end gap-2">
                     <div className="space-y-2 flex-1">
                       ...
                       <div className="h-9 px-3 flex items-center bg-white/5 border border-white/10 rounded-md text-sm text-[#D4AF37] font-bold">
                         {item.totalWeightGrams} g
                       </div>
                     </div>
                     {items.length > 1 && (
                       <Button variant="ghost" size="icon" className="h-9 w-9 text-red-400 hover:bg-red-500/10" onClick={() => removeItem(item.id)}>
        ...
        <Card className="bg-white/5 border-white/10">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center space-x-2 p-3 rounded bg-white/5 border border-white/5">
              ...
              <Label htmlFor="owner" className="cursor-pointer text-white">I confirm that I am the beneficial owner of this gold.</Label>
            </div>
          ...
          <CardContent className="space-y-6">
            <RadioGroup value={deliveryMethod} onValueChange={(v) => setDeliveryMethod(v as any)} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <Label
                  htmlFor="walk-in"
                  className="flex flex-col items-center justify-between rounded-md border-2 border-white/10 bg-white/5 p-4 hover:bg-white/10 hover:text-white peer-data-[state=checked]:border-[#D4AF37] peer-data-[state=checked]:text-[#D4AF37] cursor-pointer h-full text-center"
                >
                  <span className="mb-2 text-lg">🚶</span>
                  <span className="font-semibold">I will bring it</span>
                  <span className="text-xs text-white/40 mt-1">Personal drop-off</span>
                </Label>
              </div>
              <div>
                ...
                >
                  ...
                  <span className="text-xs text-white/40 mt-1">Logistics arranged by me</span>
                </Label>
              </div>
              <div>
                ...
                >
                  ...
                  <span className="text-xs text-white/40 mt-1">Armored transport</span>
                </Label>
              </div>
            </RadioGroup>
        ...
        <Card className="bg-white/5 border-white/10">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 border-2 border-dashed border-white/10 rounded-lg hover:border-[#D4AF37]/50 transition-colors text-center cursor-pointer relative">
                ...
                <p className="text-xs text-white/40">Proof of purchase</p>
              </div>
              <div className="p-4 border-2 border-dashed border-white/10 rounded-lg hover:border-[#D4AF37]/50 transition-colors text-center cursor-pointer relative">
                 ...
                 <p className="text-sm font-medium text-white">Upload Certificate</p>
                 <p className="text-xs text-white/40">Assay / Purity</p>
              </div>
            </div>
            ...
            {documents.length > 0 && (
              <div className="space-y-2 mt-4">
                {documents.map((doc) => (
                  <div key={doc.id} className="flex items-center justify-between p-2 rounded bg-white/5 border border-white/5 text-sm">
                    <span className="text-white flex items-center gap-2">
                       <span className="text-xs px-2 py-0.5 rounded bg-white/10 text-white/60">{doc.type}</span>
                       {doc.name}
                    </span>
                    <Button variant="ghost" size="icon" className="h-6 w-6 text-white/40 hover:text-white" onClick={() => removeDocument(doc.id)}>
        ...
        <div className="space-y-4 pt-4">
           <div className="flex items-start space-x-2">
              ...
              <div className="grid gap-1.5 leading-none">
                <Label htmlFor="lien" className="text-sm font-medium leading-none text-white cursor-pointer">
            ...
            <div className="flex items-start space-x-2">
              ...
              <div className="grid gap-1.5 leading-none">
                ...
                <p className="text-xs text-white/40">Estimated storage fee applies upon acceptance.</p>
              </div>
            </div>
        </div>

        <div className="flex gap-4 pt-4">
          <Button onClick={handleSubmit} className="flex-1 bg-[#D4AF37] text-black hover:bg-[#D4AF37]/90 font-bold h-12">
            Submit Deposit Request
          </Button>
          <Button onClick={onCancel} variant="outline" className="flex-1 border-white/10 hover:bg-white/5 text-white h-12">
            Cancel
          </Button>
        </div>
